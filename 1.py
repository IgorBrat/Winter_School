...
# fresh_fishes: dict[str: List[Fish]]

class FishShop:
    frozen_fish_boxes = {}
    fresh_fishes = {}
#    fish_names = ["salmon", "korop", "okun"]
    salmon_boxes = []
    korop_boxes = []
    okun_boxes = []
    salmons = []
    ...
    def add_fish(self, fish: Fish) -> None:
        if fish.fish_info.name=="salmon":
            ...
            self.fresh_fishes[fish.fish_info.name] = self.salmons
        if fish.fish_info.name=="korop":
            self.korops.append(fish)
            self.fresh_fishes[fish.fish_info.name] = self.korops
        if fish.fish_info.name=="okun":
            self.okuns.append(fish)
            self.fresh_fishes[fish.fish_info.name] = self.okuns
